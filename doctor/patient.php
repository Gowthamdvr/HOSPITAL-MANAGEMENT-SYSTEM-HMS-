...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="patient.php" class="nav-link-custom active"><i class="bi bi-people-fill"></i> My Patients</a>
    ...
    <div class="main-content">
        <div class="mb-4">
            <h2 class="fw-bold m-0">My Patients</h2>
            <p class="text-muted">History of patients you have treated</p>
        </div>
                ...
                <table class="table table-hover m-0">
                    <thead class="bg-light">
                        <tr>
                            ...
                            <th class="border-0 px-4 py-3">NIC</th>
                            <th class="border-0 px-4 py-3">Contact</th>
                            <th class="border-0 px-4 py-3">Last Visit</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $query = "SELECT DISTINCT patient.*, MAX(schedule.scheduledate) as last_visit 
                                  FROM patient 
                                  JOIN appointment ON patient.pid = appointment.pid 
                        ...
                        if($res->num_rows > 0){
                            while($row = $res->fetch_assoc()){
                                echo "<tr>
                                    <td class='px-4 fw-semibold'>".$row['pname']."</td>
                                    <td class='px-4'>".$row['pnic']."</td>
                                    <td class='px-4'>".$row['ptel']."</td>
                                    <td class='px-4'>".$row['last_visit']."</td>
                                </tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4' class='text-center py-5 text-muted'>No patient history found.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
