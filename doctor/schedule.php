...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="schedule.php" class="nav-link-custom active"><i class="bi bi-calendar-event-fill"></i> My Sessions</a>
    ...
    <div class="main-content">
        <div class="mb-4">
            <h2 class="fw-bold m-0">My Sessions</h2>
            <p class="text-muted">List of your scheduled availability sessions</p>
        </div>
                ...
                <table class="table table-hover m-0">
                    <thead class="bg-light">
                        <tr>
                            ...
                            <th class="border-0 px-4 py-3">Max Patients</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $query = "SELECT * FROM schedule WHERE docid=$docid ORDER BY scheduledate DESC";
                        $res = $database->query($query);
                        if($res->num_rows > 0){
                            while($row = $res->fetch_assoc()){
                                echo "<tr>
                                    <td class='px-4 fw-semibold'>".$row['title']."</td>
                                    <td class='px-4'>".$row['scheduledate']."</td>
                                    <td class='px-4'>".$row['scheduletime']."</td>
                                    <td class='px-4'>".$row['nop']."</td>
                                </tr>";
                            }
                        } else {
                            echo "<tr><td colspan='4' class='text-center py-5 text-muted'>No sessions scheduled yet.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
