...
<head>
    ...
    <title>Manage Appointments | Edoc</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="appointment.php" class="nav-link-custom active"><i class="bi bi-clock-fill"></i> Appointment</a>
    ...
    <div class="main-content">
        <div class="mb-4">
            <h2 class="fw-bold m-0">Appointments</h2>
            <p class="text-muted">View and monitor all patient bookings</p>
        </div>
        ...
        <div class="card card-custom overflow-hidden">
            <div class="table-responsive">
                <table class="table table-hover m-0">
                    ...
                    <tbody>
                        <?php
                        $list = $database->query("SELECT appointment.*, patient.pname, doctor.docname, schedule.title, schedule.scheduledate FROM appointment JOIN patient ON appointment.pid = patient.pid JOIN schedule ON appointment.scheduleid = schedule.scheduleid JOIN doctor ON schedule.docid = doctor.docid ORDER BY apponum DESC");
                        if($list->num_rows > 0){
                            while($row = $list->fetch_assoc()){
                                echo "<tr>
                                    <td><span class='fw-bold'>#".$row['apponum']."</span></td>
                                    <td>".$row['pname']."</td>
                                    <td>".$row['docname']."</td>
                                    <td>".$row['title']."</td>
                            ...
                            echo "<tr><td colspan='5' class='text-center py-5 text-muted'>No appointments booked yet.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
